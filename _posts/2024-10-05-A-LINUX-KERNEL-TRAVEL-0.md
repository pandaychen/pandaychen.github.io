...
以`4.11.6` 内核为例，与读端的 `hlist_for_each_entry_rcu` 宏相对应的**写端（修改场景）**函数，包含了增加、删除、替换等，共同点是负责以原子可见性的方式更新指针，并确保在不同 CPU 架构上通过内存屏障维持正确的执行顺序
...
{
	rb->__rb_parent_color = rb_color(rb) | (unsigned long)p; /* 保留颜色位，替换父节点指针　*/
}
...
与普通的红黑树实现不同，内核的红黑树有几个显著的设计差异：

1. **不提供查找和比较函数**：内核的 rbtree 是一个"半成品"容器，只提供插入后的平衡（`rb_insert_color`）和删除（`rb_erase`）操作，比较逻辑和查找逻辑由调用者自行实现（因为内核中不同子系统的 key 类型千差万别，用函数指针回调会带来不必要的间接调用开销）
2. **颜色信息复用指针低位**：如前文所述，`__rb_parent_color` 将父节点指针和颜色位合并存储，节省了一个字段的内存
3. **支持增幅树（Augmented RBTree）**：通过 `rb_augment_callbacks` 回调机制，允许在旋转和变色时维护额外的子树聚合信息（如 `rb_subtree_gap`）
4. **支持无锁查找（Lockless Lookup）**：配合 RCU 机制，允许在不持锁的情况下安全遍历红黑树

####	rbtree：内核的若干细节（lockless lookup）
内核红黑树支持在 RCU 保护下进行无锁查找。其核心思想是：查找操作只沿着左/右子指针走，不修改树结构，因此可以和写操作（插入/删除/旋转）并发执行，前提是写操作使用 `rcu_assign_pointer` 更新指针，读操作使用 `rcu_dereference` 读取指针

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/include/linux/rbtree.h#L64
// RCU 安全的左右子节点访问宏
...
在 `lib/rbtree.c` 的旋转操作中，内核使用了特殊的指针赋值来保证 RCU 读者的安全：

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/lib/rbtree.c
// 左旋时，旧节点的右子变为新的子树根
// 写者使用 WRITE_ONCE 保证指针赋值的原子可见性
// 读者通过 rcu_dereference(node->rb_left) 或 rcu_dereference(node->rb_right) 安全读取
```

典型的 lockless lookup 使用模式（示例）：

```cpp
// 在 RCU 读临界区内遍历红黑树
struct my_node *my_rbtree_search(struct rb_root *root, long key)
{
    struct rb_node *node;
    ...
    while (node) {
        ...
        if (key < data->key)
            node = rcu_dereference(node->rb_left);
        else if (key > data->key)
            node = rcu_dereference(node->rb_right);
        else {
            rcu_read_unlock();
            return data;
        }
...
需要注意的是，lockless lookup 有一些限制：

- 读者可能看到旋转过程中的中间状态，但由于查找只沿单一路径向下走，中间状态不会导致读者访问已释放的内存（旋转不会断开正在被遍历的路径）
- 写者仍然需要外部锁（如 `spin_lock`）来保证互斥
- 查找到的节点需要通过额外机制（如引用计数）保证生命周期

####	rbtree：内核的若干细节（augment）
增幅红黑树（Augmented RBTree）是内核红黑树的一个扩展机制，**允许在每个节点上维护关于其子树的聚合信息**。最典型的应用就是 `vm_area_struct` 中的 `rb_subtree_gap` 字段

增幅树的核心问题是：当红黑树进行旋转或节点替换时，子树结构发生了变化，聚合信息必须同步更新。内核通过 `rb_augment_callbacks` 回调结构来解决：

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/include/linux/rbtree_augmented.h#L44
...
| 回调 | 触发时机 | 职责 |
|------|---------|------|
| `propagate` | 节点值变化后，沿路径向上传播 | 从 `node` 开始，向上直到 `stop`，逐级重新计算聚合值 |
| `copy` | 节点被替换时（如 `rb_erase` 中后继替换） | 将旧节点的聚合信息拷贝到新节点 |
| `rotate` | 左旋或右旋时 | 旋转后，`old` 成为 `new` 的子节点，需先更新 `old` 的聚合值，再更新 `new` 的聚合值 |

以 VMA 的 `rb_subtree_gap` 为例，内核在 `mm/mmap.c` 中定义了具体的回调实现：

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/mm/mmap.c#L335
// 计算单个节点的 subtree_gap
static long vma_compute_subtree_gap(struct vm_area_struct *vma)
{
    unsigned long max = vma->vm_start;
    // 当前节点与前驱之间的 gap
    if (vma->vm_prev)
        max -= vma->vm_prev->vm_end;
    // 与左子树的最大 gap 取最大值
    if (vma->vm_rb.rb_left) {
        unsigned long subtree_gap =
...
// propagate 回调：从 node 向上到 stop 逐级更新 rb_subtree_gap
static void vma_gap_callbacks_propagate(struct rb_node *rb, struct rb_node *stop)
{
    while (rb != stop) {
        ...
        if (node->rb_subtree_gap == augmented)
            break;  // 剪枝：值未变化则无需继续向上
        node->rb_subtree_gap = augmented;
        rb = rb_parent(&node->vm_rb);
    }
}

// rotate 回调：旋转后先更新降级节点（old），再更新升级节点（new）
static void vma_gap_callbacks_rotate(struct rb_node *rb_old, struct rb_node *rb_new)
{
    ...
    // old 现在是 new 的子节点，先算 old
    old->rb_subtree_gap = vma_compute_subtree_gap(old);
    // 再算 new（包含了 old 的信息）
    new->rb_subtree_gap = vma_compute_subtree_gap(new);
}
```

增幅树使用时，不能直接调用 `rb_insert_color` 和 `rb_erase`，而要使用带 `_augmented` 后缀的版本：

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/include/linux/rbtree_augmented.h
...
1. **BST 插入**：调用者自行完成二叉搜索树的标准插入（找到位置、`rb_link_node`），新节点初始为红色
2. **平衡修复**：调用 `rb_insert_color` 修复可能违反的红黑树性质
...
阶段二 `rb_insert_color` 的核心实现（[源码](https://elixir.bootlin.com/linux/v4.11.6/source/lib/rbtree.c#L92)）：

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/lib/rbtree.c#L92
void rb_insert_color(struct rb_node *node, struct rb_root *root)
{
    __rb_insert(node, root, dummy_rotate);
}

// __rb_insert 的核心逻辑（简化注释版）
    ...
    void (*augment_rotate)(struct rb_node *old, struct rb_node *new))
{
    ...
    while (true) {
        ...
        // Case 2: 父节点是黑色 → 无需修复，红黑树性质未被破坏
        if (rb_is_black(parent))
            break;

        gparent = rb_red_parent(parent);

        // 父节点是红色，需要修复（违反"红色节点的子节点必须是黑色"）
        tmp = gparent->rb_right;
        if (parent != tmp) {    // parent == gparent->rb_left
            if (tmp && rb_is_red(tmp)) {
                // Case 3: 叔叔节点也是红色
                // 将父和叔都染黑，祖父染红，然后对祖父继续向上修复
            ...
            tmp = parent->rb_right;
            if (node == tmp) {
                // Case 4: 叔叔是黑色，且当前节点是右子（LR 情况）
                // 先对 parent 左旋，转化为 Case 5
                tmp = node->rb_left;
            ...
            // Case 5: 叔叔是黑色，当前节点是左子（LL 情况）
            // 对 gparent 右旋，parent 变成子树根
            WRITE_ONCE(gparent->rb_left, tmp);  // gparent 的左子改为 parent 的右子
            ...
            // 镜像情况（parent == gparent->rb_right），逻辑对称
            // ...（省略，与上面左右对称）
        }
    }
}
...
| Case | 条件 | 操作 | 是否终止 |
|------|------|------|---------|
| Case 3 | 叔叔是红色 | 父+叔染黑，祖父染红，对祖父继续循环 | 否，向上传播 |
| Case 4 | 叔叔是黑色，LR/RL | 对父旋转，转化为 Case 5 | 否，进入 Case 5 |
| Case 5 | 叔叔是黑色，LL/RR | 对祖父旋转，重新着色 | 是 |

####	核心代码解析：删除
红黑树的删除比插入更复杂。`rb_erase` 先执行标准 BST 删除（找后继替换），再通过 `____rb_erase_color` 修复平衡

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/lib/rbtree.c#L275
...
阶段一：`__rb_erase_augmented`（BST 删除 + 后继替换），[源码](https://elixir.bootlin.com/linux/v4.11.6/source/include/linux/rbtree_augmented.h#L131)

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/include/linux/rbtree_augmented.h#L131
// 简化的核心逻辑
             ...
             const struct rb_augment_callbacks *augment)
{
    ...
    if (!tmp) {
        // Case A: 无左子 → 用右子直接替换
        // 如果被删节点是黑色且替换节点（child）为空或黑色，需要 rebalance
        parent = rb_parent(node);
        ...
        // 如果删除的是黑色节点，需要从 child2 开始修复
        rebalance = ... ;
    }

    // 调用 augment->propagate 从 parent 向上更新聚合信息
    augment->propagate(parent, NULL);
    return rebalance;
}
```

阶段二：`____rb_erase_color`（删除后的平衡修复），[源码](https://elixir.bootlin.com/linux/v4.11.6/source/lib/rbtree.c#L197)，这是红黑树最复杂的部分，需要处理删除黑色节点后导致的"黑色缺失"问题：

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/lib/rbtree.c#L197
// 简化的核心逻辑
static __always_inline void
    ...
    void (*augment_rotate)(struct rb_node *old, struct rb_node *new))
{
    ...
    while (true) {
        ...
        if (node != sibling) {  // node 是左子
            if (rb_is_red(sibling)) {
                // Case 1: 兄弟是红色
                // 对 parent 左旋，兄弟和父交换颜色
                // 转化为 Case 2/3/4
            ...
            if (!tmp1 || rb_is_black(tmp1)) {
                ...
                if (!tmp2 || rb_is_black(tmp2)) {
                    // Case 2: 兄弟的两个子节点都是黑色
                    // 将兄弟染红，黑色缺失上移到 parent
                    rb_set_parent_color(sibling, parent, RB_RED);
                    if (rb_is_red(parent)) {
                        rb_set_black(parent);  // parent 是红色 → 染黑即可终止
                    } else {
                        node = parent;
                        parent = rb_parent(node);
                        if (parent)
            ...
            // Case 4: 兄弟的远侄是红色
            // 对 parent 左旋，兄弟继承 parent 颜色，parent 和远侄染黑
            WRITE_ONCE(parent->rb_right, sibling->rb_left);
            WRITE_ONCE(sibling->rb_left, parent);
...
| Case | 条件 | 操作 | 是否终止 |
|------|------|------|---------|
| Case 1 | 兄弟是红色 | 对父旋转，转化为 Case 2/3/4 | 否 |
| Case 2 | 兄弟的两子都是黑色 | 兄弟染红，黑色缺失上移 | 父是红 → 是；父是黑 → 否 |
| Case 3 | 兄弟的远侄黑、近侄红 | 对兄弟旋转，转化为 Case 4 | 否 |
| Case 4 | 兄弟的远侄是红色 | 对父旋转 + 重新着色 | 是 |

内核实现中一个重要的性能细节：`__always_inline` 确保这些函数被内联展开，避免函数调用开销。同时在旋转操作中使用 `WRITE_ONCE` 保证指针修改的原子可见性，兼容 lockless lookup 的读者
...
##	0x04	vm_area_struct：rb_subtree_gap实现分析
...
`rb_subtree_gap` 必须在红黑树结构发生变化时（插入、删除、旋转）同步更新，否则查找空闲区间会得到错误结果。内核通过前文介绍的增幅红黑树回调机制自动维护

VMA 红黑树使用的增幅回调通过宏 `RB_DECLARE_CALLBACKS` 生成，定义在 `mm/mmap.c` 中：

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/mm/mmap.c#L381
// 用宏自动生成 propagate / copy / rotate 三个回调函数
RB_DECLARE_CALLBACKS(static, vma_gap_callbacks, struct vm_area_struct, vm_rb,
		     unsigned long, rb_subtree_gap, vma_compute_subtree_gap)
```

展开后等价于如下三个回调，注册到 `struct rb_augment_callbacks vma_gap_callbacks` 中：

**1. propagate 回调：沿路径向上传播更新**

当某个节点的 gap 值可能变化（如 VMA 被插入/删除/修改了地址范围），从该节点开始沿父指针向上逐级重新计算 `rb_subtree_gap`，直到值不再变化（剪枝优化）或到达根节点：
...
static void vma_gap_callbacks_propagate(struct rb_node *rb, struct rb_node *stop)
{
    while (rb != stop) {
        struct vm_area_struct *node =
            rb_entry(rb, struct vm_area_struct, vm_rb);
        unsigned long augmented = vma_compute_subtree_gap(node);
        // 剪枝：如果重新计算的值与当前值相同，说明上层也不会变，提前终止
        if (node->rb_subtree_gap == augmented)
            break;
        node->rb_subtree_gap = augmented;
        rb = rb_parent(&node->vm_rb);
...
**2. copy 回调：节点替换时拷贝聚合值**

在 `rb_erase_augmented` 中，如果被删除的节点有两个子节点，会用中序后继替换。此时需要将被删节点的 `rb_subtree_gap` 拷贝到后继节点（后继节点将占据被删节点的树位置）：

```cpp
static void vma_gap_callbacks_copy(struct rb_node *rb_old, struct rb_node *rb_new)
{
    struct vm_area_struct *old =
        rb_entry(rb_old, struct vm_area_struct, vm_rb);
    struct vm_area_struct *new =
        rb_entry(rb_new, struct vm_area_struct, vm_rb);
    // 新节点继承旧节点的子树 gap 值
    new->rb_subtree_gap = old->rb_subtree_gap;
}
```

**3. rotate 回调：旋转后更新涉及的两个节点**

旋转会改变父子关系，旋转后 `old` 降级为 `new` 的子节点。必须先更新 `old`（现在是子节点，子树变小了），再更新 `new`（现在是子树根，子树变大了），顺序不能反：
...
以 VMA 插入为例（[`vma_link`](https://elixir.bootlin.com/linux/v4.11.6/source/mm/mmap.c#L643)），完整的更新链路为：

```text
vma_link()
  ├── __vma_link_rb()
  │     ├── rb_link_node()          // BST 插入，节点初始为红色
  │     ├── vma->rb_subtree_gap = 0 // 初始化新节点的 gap
  │     └── rb_insert_augmented()   // 平衡修复 + 增幅更新
  │           ├── __rb_insert()
  │           │     ├── 旋转时调用 rotate 回调
  │           │     └── 变色时不需要回调
  │           └── propagate 回调从插入点向上传播
  └── vma_gap_update()              // 显式更新 gap 并向上传播
```

上面流程中对应的核心代码如下：

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/mm/mmap.c#L537
...
//https://elixir.bootlin.com/linux/v4.11.6/source/mm/mmap.c#L398
...
//https://elixir.bootlin.com/linux/v4.11.6/source/include/linux/rbtree_augmented.h#L57
static inline void
...
//https://elixir.bootlin.com/linux/v4.11.6/source/lib/rbtree.c#L457
...
在上面代码中`augment_rotate`即为`vma_gap_callbacks`，是宏定义的`rb_augment_callbacks`结构

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/include/linux/rbtree_augmented.h#L92
...
`vma_gap_update` 的实现很简洁：

```cpp
// https://elixir.bootlin.com/linux/v4.11.6/source/mm/mmap.c#L389
static void vma_gap_update(struct vm_area_struct *vma)
{
    // 重新计算当前节点的 rb_subtree_gap 并触发 propagate
    vma_gap_callbacks_propagate(&vma->vm_rb, NULL);
}
```

小结：`rb_subtree_gap` 的更新不是每次都全树重算，而是利用增幅回调机制做**最小化的局部更新**——只沿受影响的路径向上传播，配合剪枝优化（值未变则停止），使得维护开销保持在 `O(logN)` 级别
...
原因是唤醒（`wake_up`）并不等于条件一定满足，存在以下几种"虚假唤醒"场景：

- **惊群效应（Thundering Herd）**：多个进程等待同一个 waitqueue，`wake_up` 可能唤醒全部或多个等待者，但条件资源（如一个空闲 buffer）只够满足其中一个，其余被唤醒的进程必须重新检查条件并继续睡眠
- **信号中断**：对于 `wait_event_interruptible` 系列，进程可能因收到信号而被唤醒，此时 `condition` 并未变化
- **竞态条件（Race Condition）**：从唤醒到进程实际获得 CPU 执行之间有时间窗口，在这个窗口内条件可能又被其他进程消耗掉了（例如某个 buffer 刚被填满又立即被另一个进程读空了）
- **非排他唤醒**：`wake_up` 默认唤醒 waitqueue 上的所有非排他等待者（non-exclusive），这些进程竞争同一个条件，只有最先抢到的能真正满足条件

因此，`___wait_event` 中的 `for(;;)` 循环本质上是一个**自旋等待模式**：唤醒后先验证条件，不满足就继续调用 `schedule` 让出 CPU。这个"唤醒-检查-再睡"的模式是 waitqueue 正确性的基石

2、关于`wait_event*`函数中`condition`的理解，`condition`是谁负责更新的？

`condition` 由 **waker（唤醒方）** 负责在调用 `wake_up` 之前更新。典型模式是：

```cpp
// waker 端（生产者/事件产生方）
some_shared_flag = true;          // ① 先更新 condition
wake_up(&my_wait_queue);          // ② 再唤醒等待者
```

```cpp
// waiter 端（消费者/等待方）
wait_event(my_wait_queue, some_shared_flag == true);
```

这里有两个要点：

- `condition` 通常是一个共享变量的表达式（如 `buffer_ready == true`、`list_empty(&queue) == false`），它被编译器在 `___wait_event` 宏的 `for` 循环中**每次重新求值**，而非一次性取值
- waker 必须保证**先修改 condition 对应的共享变量，再调用 `wake_up`**。如果顺序反过来（先唤醒再设条件），waiter 被唤醒后检查 condition 仍为 false，会继续睡眠，而此后再也没有人来唤醒它，导致永久休眠。对于 SMP 环境，waker 端需确保写操作对其他 CPU 可见（`wake_up` 内部包含了必要的内存屏障）
