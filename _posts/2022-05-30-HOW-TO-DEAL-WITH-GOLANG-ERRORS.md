...
-	无论是 `Wrap`， `WithMessage` 或 `WithStack` ，当传入的 err 参数为 `nil` 时， 都会返回 `nil`， 这意味着在调用此方法之前无需作 `nil` 判断，保持了代码简洁
...
不过，这种方式不一定能够满足业务的需求。即默认错误处理方式非常简单直白， 但是有个很大的问题就是 表达能力非常有限。 因为使用类似于 HTTP 状态码的有限抽象 code 没法表达出多样的业务层的错误，且 message 这种字符串也是不应该被请求方当做业务错误标识符来使用。所以需要一个额外能够传递业务错误码甚至更多额外错误信息字段的功能。
