...
####    内核通知机制与优化
Linux 内核在保障数据及时到达与避免频繁唤醒导致的上下文切换开销之间，是如何做权衡的？从底层协议栈的代码来看，每当一个按序到达的数据包写入 `sk_receive_queue` 后，内核确实都会调用 `sk->sk_data_ready()`，但是这不等于应用层会立刻收到通知或被唤醒。内核在 `sk_data_ready` 之后，设计了多层压制与合并策略，核心内容如下：


1、（协议栈视角）按序到达才触发

在TCP状态机处理时，并不是所有的 TCP 包都有资格调用 `sk_data_ready`：

-   乱序包（Out-of-Order）： 如果网卡收到了一个序列号跳跃的包（网络丢包或乱序），内核会把它放入 out_of_order_queue（乱序队列），此时内核绝不会调用 `sk_data_ready`
-   只有填补了空洞（Gap）之后，有当那个缺失的包终于到达，内核把乱序队列里的包拼接回 `sk_receive_queue`、且 TCP 的 `rcv_nxt`（期望接收的下一个序列号）向前推进时，才会触发 `sk_data_ready`

2、内核到应用层的低水位抑制（`SO_RCVLOWAT`）

即使数据进入了 `sk_receive_queue` 且调用了 `sk_data_ready`，如果开启了`SO_RCVLOWAT`特性（低水位线、Receive Low Watermark机制），内核依然可以不通知应用层

-   默认行为：`SO_RCVLOWAT` 默认是 `1` 字节。这意味着只要来了一个包，就会立刻唤醒正在阻塞 read 的线程，或者触发 epoll
-   高性能优化策略：对于高性能网关场景，每次需要处理大块业务数据（`4KB`），可以通过 `setsockopt` 设置 `SO_RCVLOWAT = 4096`来优化通知场景
-   内核如何压制？当 `sk_data_ready` 触发后，内核会去检查当前 `sk_receive_queue` 里的数据总大小，如果少于 `4096` 字节，阻塞在 `recv` 上的进程将继续睡眠，epoll 也绝对不会向用户态上报 EPOLLIN 事件；只有当积压的数据终于超过了 `4096` 字节，内核才会真正执行唤醒。这极大地减少了高吞吐场景下，进程被频繁唤醒又发现数据不够时的上下文切换开销

3、网卡与软中断视角的批量合并（GRO/NAPI）：协议栈视角下，在进入 TCP 层之前，内核在网卡和驱动层就已经做了一次**欺上瞒下**的合并通知

-   NAPI (New API) 机制： 网卡收到包后不立刻触发硬中断，而是通过软中断以轮询（Poll）方式一次性从网卡 Ring Buffer 摘下一批包（如 `64` 个包）
-   GRO (Generic Receive Offload) 技术： 在软中断把这批包往协议栈送的时候，内核会启动 GRO。如果发现这几个连续的包属于同一个 TCP 连接，内核会在底层合并成一个超级大的巨型包（`sk_buff`）
-   当这个巨型包进入 TCP 层并写入 `sk_receive_queue` 时，内核只调用了 `1` 次 `sk_data_ready`（原本可能引发多次唤醒的小包，在网卡边界就被无形地压缩成了 `1` 次通知）

4、Socket 锁状态引发的延迟通知（`sk_backlog`机制）

-   如果软中断到达时，用户态进程正在霸占 socket（`owned == 1`），为了提高收包性能，数据包会被扔进 `sk_backlog`。注意此时软中断绝对不会调用 `sk_data_ready`
-   那么谁来通知呢？当用户态进程调用 `release_sock` 退出时，它会在自己的上下文里处理 backlog 并把包挪到 `sk_receive_queue`
-   既然是用户态进程挪过去的，它自己自然知道有数据了，此时它根本不需要外部的异步唤醒通知，直接顺理成章地在系统调用里把数据带回用户态即可。这种设计在并发冲突时，天然消灭了通知开销

5、I/O 复用层（Epoll）的边缘触发策略（ET），即epoll的LT和ET触发策略

-   水平触发 (LT)： 只要 `sk_receive_queue` 里有数据，每次调用 `epoll_wait`，内核都会孜孜不倦地通知应用层，直到应用层把数据读完
-   边缘触发 (ET)： 只有当 Socket 的状态发生变化时（比如从无数据变成有数据，或者又有新数据追加进来），`sk_data_ready` 才会通过 `ep_poll_callback` 将该 Socket 挂入 epoll 的就绪链表（`rdllist`）
-   ET模式下，如果内核通知了应用层，应用层调用 `recv` 只读了一半数据就收手了，那么只要没有新包进来，`sk_data_ready` 就不会再被调用，epoll 就再也不会通知应用层了，哪怕队列里还剩下一半数据。这种策略逼迫应用层必须用一个 `while(true)` 循环一直读到 `EAGAIN`，从而实现了全吞吐的高效收包
