...
var (
	...
	// m 是scheme和resolver构建器的关系映射，scheme可以是我们自定义的，如etcd、consul等string
...
// 注册各种scheme对应的resolver构建器，注册自定义的resolver
...
type Address struct {
	...
	// 地址名。当用于authentication时，它是grpc负载均衡器的名称
	ServerName string
	// Metadata is the information associated with Addr, which may be used
	// to make load balancing decision.
	// 地址关联元数据信息，被用于做负载均衡决策，一般关联一个map
	Metadata interface{}
}
...
// gRPC的客户端核心概念：ClientConn，是一个方法的集合，是解析器Resolver通知ClientConn解析结果的回调接口
...
// 对应的就是该详细设计的名称语法（如上面的dns://8.8.8.8/www.qq.com）
type Target struct {
	Scheme    string
	Authority string
	Endpoint  string
...
type Builder interface {
	...
	// not nil.
	// 构建自定义解析器，必须实现Build方法，比如下面一个小节分析的dnsBuilder，就实现了这两个方法，
	// 一般在Build()方法中，会包含实现watcher的逻辑
	Build(target Target, cc ClientConn, opts BuildOption) (Resolver, error)
	// Scheme returns the scheme supported by this resolver.
	// Scheme is defined at https://github.com/grpc/grpc/blob/master/doc/naming.md.
	// 指定构建的是那种Scheme类型的解析器，就是客户端调用的scheme名字
	Scheme() string
}
...
type Resolver interface {
	...
	ResolveNow(ResolveNowOption)		//通过channel方式，唤醒select，强制执行立即解析
...
const (
	...
	// 使用dns TXT记录grpc_config属性发布服务配置，TXT记录必须满足下面的格式（均衡器）
...
// 要构建自己的负载均衡器，必须实现Build方法，该方法返回一个resolver.Resolver，通常该方法中会开启一个子协程，用来watch（or pull）服务地址的变化
// 这里先埋一个问题，resolver的Builder是在哪里调用的？
func (b *dnsBuilder) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOption) (resolver.Resolver, error) {
	...
	// DNS address (non-IP). --域名方式（与IP是区分开的）
...
// dnsResolver，监视P目标的名称解析更新，并通过channel通知上层gRPC的接口实时更新,实现grpc-lb需要实现此结构
type dnsResolver struct {
	freq   time.Duration		//解析周期（PULL）
	host   string				//待解析的域名（如www.qq.com）
	port   string
	...
	// rn channel is used by ResolveNow() to force an immediate resolution of the target.
	// rn该channel被用于强制触发ResolveNow()的立即执行
	rn chan struct{}
	...
	//被用于等待watcher()协程执行完毕后再强制Close()，否则会出现watcher()协程和replaceNetFunc函数间数据竞争问题
	wg sync.WaitGroup
}

// ResolveNow invoke an immediate resolution of the target that this dnsResolver watches.
// 往rn发送执行立即解析信号（这里多说两句：这个channel用法是golang中很常见的技巧，注意看watcher()中的select分支，有一个case <-d.rn:条件，
// 当调用ResolveNow时，会立即触发执行DNS解析，这里的default是防止channel写入阻塞）
...
func (d *dnsResolver) Close() {
	//取消解析过程，取消成功后ctx会收到Done()信号
	d.cancel()

	//等待所有协程退出（等待watcher()协程执行完毕）
	d.wg.Wait()
...
// gRPC-lb功能的第三核心实现：watcher（一般以一个单独的子routine创建）
func (d *dnsResolver) watcher() {
	defer d.wg.Done()		// 协程退出， 通知watcher()协程执行完毕
	for {
		select {
		case <-d.ctx.Done():
			return
		...
		result, sc := d.lookup()	//主动调用DNS解析流程，start DNS resovle
		// Next lookup should happen after an interval defined by d.freq.
		// 重置定时器
		d.t.Reset(d.freq)
		d.cc.NewServiceConfig(string(sc))	//发送服务配置通知	
		d.cc.NewAddress(result)				//发送地址列表通知gRPC更新（后端endpoint列表），NewAddress已经被UpdateState方法代替了
	}
}


// 查找SRV记录，用于查询到的均衡器地址列表
func (d *dnsResolver) lookupSRV() []resolver.Address {
	var newAddrs []resolver.Address
	//1. 先根据SRV记录，获取grpclb的记录（获取所有负载均衡节点名称）
	...
	// 2. 根据上一步获取的均衡节点名称，查找对应的负载均衡器地址列表（A记录）
	for _, s := range srvs {
...
//	查找TXT解析记录，返回查询到的服务配置（注意必须以此为前缀：txtAttribute = "grpc_config="）
...
// 查找DNS的A记录，用于返回查询到的后台服务器地址列表
func (d *dnsResolver) lookupHost() []resolver.Address {
	var newAddrs []resolver.Address
	//调用net的函数lookupHost发起DNS查询
...
// DNS解析与获取结果，返回地址列表（服务器地址和负载均衡器地址）和服务配置
