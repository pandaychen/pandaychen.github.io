...
flowchart TD
    ...
    subgraph syscall_entry ["系统调用入口层"]
        S1["SYSCALL_DEFINE3/4"]
        S2["do_sys_open"]
        S3["build_open_flags: 参数解析"]
        S4["getname: 路径拷贝到内核"]
        S5["get_unused_fd_flags: 分配 fd"]
    end
    subgraph vfs_layer ["VFS 层: 路径查找 + 文件打开"]
        V1["do_filp_open"]
        V2["path_openat: RCU-walk"]
        V3["path_openat: ref-walk"]
        V4["path_openat: REVAL"]
    end
    subgraph path_resolve ["path_openat 内部"]
        P1["path_init: 确定起点"]
        P2["link_path_walk: 逐级解析中间分量"]
        P3["do_last: 处理最后分量"]
        P4["trailing_symlink: 跟随尾部链接"]
    end
    ...
    P3 -->|"symlink: 返回>0"| P4
    P4 --> P2
    P3 -->|"成功: 返回0"| R1["vfs_open -> 文件系统 open"]
```
...
```mermaid
flowchart TD
    A["path_openat 入口"] --> B["get_empty_filp: 分配空 file"]
    B --> C["path_init: 初始化 nameidata"]
    C --> D["进入 while 循环"]
    D --> E["link_path_walk: 解析中间分量"]
    E -->|"成功: 返回0"| F["do_last: 处理最后分量"]
    E -->|"错误"| G["terminate_walk + 错误处理"]
    F -->|"返回0: 文件已打开"| H["成功返回 file"]
    F -->|"返回>0: 遇到symlink"| I["trailing_symlink"]
    F -->|"返回<0: 错误"| G
    I -->|"获取链接目标 s"| D
    I -->|"错误"| G
    G --> J{"错误码?"}
    J -->|"-ECHILD / -ESTALE"| K["返回错误给 do_filp_open 触发重试"]
    J -->|"其他"| L["返回错误"]
```

####	link_path_walk 内部流程图

```mermaid
flowchart TD
    A["link_path_walk 入口"] --> B["跳过前导斜杠 /"]
    B --> C["取下一个分量"]
    C --> D["hash_name: 计算哈希值"]
    D --> E["设置 nd->last 和 nd->last_type"]
    E --> F{"分量后面还有 / ?"}
    F -->|"否: 最后分量"| G["返回0: 交给 do_last"]
    F -->|"是: 中间分量"| H["may_lookup: 权限检查"]
    H --> I["walk_component"]
    I --> J["handle_dots?"]
    J -->|"是 . / .."| K["handle_dots: 处理特殊分量"]
    J -->|"否: 普通分量"| L["lookup_fast: dcache 查找"]
    L -->|"命中"| M["step_into"]
    L -->|"未命中"| N["lookup_slow: 文件系统查找"]
    N --> M
    L -->|"错误 -ECHILD"| O["返回错误"]
    M --> P{"d_is_symlink?"}
    P -->|"否"| Q["path_to_nameidata: 更新 nd"]
    P -->|"是"| R["pick_link: 处理符号链接"]
    R --> S["get_link: 获取链接目标"]
    S --> C
    Q --> T{"d_mountpoint?"}
    T -->|"是"| U["__follow_mount_rcu / follow_managed"]
    T -->|"否"| C
    U --> C
    K --> C
```
...
```mermaid
flowchart TD
    A["do_last 入口"] --> B{"nd->last_type?"}
    B -->|"LAST_DOTDOT"| C["handle_dots -> follow_dotdot"]
    B -->|"LAST_DOT"| D["handle_dots: 不操作"]
    B -->|"LAST_NORM"| E{"O_CREAT 标志?"}

    E -->|"否: 只读/追加等"| F["lookup_fast 查找最后分量"]
    F -->|"命中"| G["finish_lookup"]
    F -->|"未命中"| H["lookup_slow"]
    H --> G

    E -->|"是: 创建文件"| I["lookup_open"]
    I --> J{"文件已存在?"}
    J -->|"是"| G
    J -->|"否"| K["vfs_create: 创建文件"]
    K --> G

    G --> L["step_into"]
    L --> M{"d_is_symlink?"}
    M -->|"是 且 LOOKUP_FOLLOW"| N["返回 > 0: 需跟随链接"]
    M -->|"否"| O{"d_is_dir 且需要文件?"}
    O -->|"是"| P["返回 -EISDIR"]
    O -->|"否"| Q["may_open: 权限检查"]
    Q --> R["vfs_open"]
    R --> S["具体文件系统 open\n如 ext4_file_open"]
...
如上，结构体`hlist_bl_node`类似一个双向链表，采用的是头插法（`hlist_bl_add_head()`）,[前文](https://pandaychen.github.io/2024/10/05/A-LINUX-KERNEL-TRAVEL-0/)已经介绍过此类内核链表实现。`pprev`这个二级指针，指向的不是前面节点，而是指向前面节点的next指针的存储地址（如果是头节点，`pprev`会指向`hlist_bl_head`的`first`的存储地址）。这样的好处是从链表中移除自身的时候，只需要将`next->pprev = pprev`即可,关联内核函数为`__hlist_bl_del()`，对`pprev`解引用操作即`(*pprev)`这个值为当前链表节点`hlist_bl_node`的地址，下面介绍关于`dentry_hashtable`的几类典型操作：
...
当创建一个新的dentry时，[`d_alloc()`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/dcache.c#L1598)会为其申请内存空间，而后通过`d_add`函数将这个新的dentry和对应的inode关联起来，并插入dcache的hash表中；对dentry的使用以`dget`和[`dput`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/dcache.c#L751)来实现引用计数`dentry->d_lockref.count`的加减，当引用计数变为`0`时，加入LRU链表，再次被使用后，从LRU链表移除。最后[`d_drop`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/dcache.c#L463)函数直接将一个dentry从hashtable中移除（如dentry失效时），而`d_delete`的目标则是归还inode，并将一个dentry置于negative状态，在内核4.11.6版本中，在`vfs_unlink`、`vfs_rmdir`等操作成功前的最后都会调用`d_delete`

dcache 核心操作函数如下：

| 函数 | 作用 | 对引用计数的影响 | 典型调用场景 |
|------|------|-----------------|-------------|
| `d_alloc` | 分配一个新 dentry，初始化各字段，`d_lockref.count=1` | 初始为 `1` | `lookup_slow` -> `d_alloc_parallel` |
| `d_add` | 将 dentry 与 inode 关联（`d_instantiate`），并插入 dcache 哈希表（`d_rehash`） | 不变 | 文件系统 `lookup` 完成后 |
| `dget` | 增加 dentry 引用计数 | `+1` | ref-walk 模式获取 dentry 引用 |
| `dput` | 减少引用计数，为 `0` 时加入 LRU 或释放 | `-1` | 路径查找完成、文件关闭 |
| `d_drop` | 从 dcache 哈希表中摘除（清除 `DCACHE_RCUACCESS` 后 `__d_drop`） | 不变 | dentry 失效、`d_invalidate` |
| `d_delete` | 若引用计数为 `1` 则调用 `dentry_unlink_inode` 归还 inode 并置为 negative；否则调用 `__d_drop` 从哈希表摘除 | 不变 | `vfs_unlink`、`vfs_rmdir` |
...
dentry 的生命周期状态机如下图所示，一个 dentry 在其生命周期中会经历以下状态转换：

```mermaid
stateDiagram-v2
    [*] --> Unused: d_alloc 分配
    Unused --> InUse: dget / 路径查找引用
    InUse --> Unused: dput 且 count=0\n加入 LRU
    InUse --> Negative: d_delete\n归还 inode
    Unused --> InUse: 再次被查找命中\n从 LRU 移除
    Unused --> Freed: 内存回收\nshrink_dcache
    Negative --> Freed: dput 且 count=0
    Negative --> InUse: 同名文件被创建\nd_splice_alias
    InUse --> Freed: d_drop + dput
    Unused --> Dropped: d_drop\n从哈希表摘除
    Dropped --> Freed: dput 且 count=0
```

-	**Unused**（未使用）：`d_lockref.count == 0`，dentry 仍在哈希表中但不被任何进程引用，位于 LRU 链表上等待回收或复用
-	**In-Use**（使用中）：`d_lockref.count > 0`，至少有一个引用者（路径查找、打开的文件等）
-	**Negative**（负 dentry）：`d_inode == NULL`，表示此路径分量曾被查找但不存在，缓存"不存在"信息以加速后续查找
-	**Dropped**（已摘除）：通过 `d_drop` 从哈希表中移除，但内存尚未释放
-	**Freed**（已释放）：内存被 slab 回收器回收
...
在路径查找过程中，VFS 通过两张全局哈希表来实现挂载点的快速定位：

-	`mount_hashtable`：以 `<parent_vfsmount, mountpoint_dentry>` 为 key 进行哈希，用于判断某个 dentry 上是否挂载了子文件系统。核心查找函数为 [`__lookup_mnt`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/mount.h#L82)，在 `lookup_fast` / `step_into` 等路径查找中被频繁调用

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namespace.c#L624
struct mount *__lookup_mnt(struct vfsmount *mnt, struct dentry *dentry)
```

-	`mountpoint_hashtable`：以 dentry 地址为 key，用于查找某个 dentry 对应的 `struct mountpoint`。核心查找函数为 [`lookup_mountpoint`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/namespace.c#L716)，主要在 `mount` 系统调用中使用

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namespace.c#L716
static struct mountpoint *lookup_mountpoint(struct dentry *dentry)
```

两张hashtable表的查找时间复杂度均为 `O(1)`（哈希冲突时退化为链表遍历）。路径查找阶段主要依赖 `mount_hashtable`，其查找路径为：`step_into` -> `handle_mounts` -> `__follow_mount_rcu` / `follow_managed` -> `__lookup_mnt`
...
上面展示了 `attach_mnt` 将子 mount 挂接到父 mount 树上的过程。从用户态 `mount(2)` 系统调用到此处的完整调用链为：

```BASH
mount(2)
  -> SYSCALL_DEFINE5(mount, ...)
    -> do_mount()
      -> do_new_mount()                # 处理新挂载请求
        -> vfs_kern_mount()            # 创建 mount 实例 + 调用文件系统的 mount 回调
          -> mount_fs()                # 调用 file_system_type->mount()，获取超级块
          -> alloc_vfsmnt()            # 分配 struct mount
        -> do_add_mount()              # 锁保护下将 mount 加入全局树
          -> graft_tree()              # 将新 mount 嫁接到挂载点
            -> attach_recursive_mnt()  # 递归挂载（支持 bind mount）
              -> mnt_set_mountpoint()  # 设置 mnt_parent / mnt_mountpoint / mnt_mp
              -> commit_tree()         # 插入 mount_hashtable + 全局链表
```

其中 `vfs_kern_mount` 负责创建 mount 实例并调用具体文件系统的 `mount` 回调（如 `ext4_mount`）获取超级块；`graft_tree` 通过 `attach_recursive_mnt` 完成 mount 树的嫁接，最终在 `commit_tree` 中将新 mount 插入 `mount_hashtable`，使后续路径查找的 `__lookup_mnt` 能够找到它
...
`path`、`vfsmount`、`dentry` 三者在路径查找中的协作关系如下：

```mermaid
graph LR
    subgraph nameidata_ctx ["nameidata（查找上下文）"]
        nd_path["nd.path"]
        nd_root["nd.root"]
    end
    subgraph path_struct ["struct path"]
        mnt_ptr["mnt -> vfsmount"]
        dentry_ptr["dentry -> dentry"]
    end
    subgraph vfsmount_struct ["struct vfsmount"]
        mnt_root["mnt_root: 文件系统根dentry"]
        mnt_sb["mnt_sb: 超级块"]
    end
    subgraph dentry_struct ["struct dentry"]
        d_inode["d_inode: 关联的inode"]
        d_parent["d_parent: 父dentry"]
        d_name["d_name: 分量名"]
    end

    nd_path --> path_struct
    nd_root --> path_struct
    mnt_ptr --> vfsmount_struct
    dentry_ptr --> dentry_struct
    mnt_root --> dentry_struct
```

在路径查找的每一步中，`nd.path` 始终维护当前走到的位置：`mnt` 指向当前所在的文件系统（如从 rootfs 穿越 mount 点到 ext4 时，`mnt` 会切换），`dentry` 指向当前分量对应的目录项。当 `walk_component` 完成一个分量的解析后，通过 `path_to_nameidata` 将新的 `<mnt, dentry>` 更新回 `nd.path`，这样下一轮循环就以此为起点继续查找下一个分量
...
3、**`nameidata`结构中`inode`与`link_inode`成员的作用**？

-	`inode`（`nd->inode`）：缓存当前查找位置 `nd->path.dentry->d_inode` 的值。在 RCU-walk 模式下，直接解引用 `dentry->d_inode` 可能与 seqcount 产生竞态，因此在 `lookup_fast` 成功后将 inode 缓存到 `nd->inode`，后续权限检查（`inode_permission`）、类型判断（`d_can_lookup`）等都使用此缓存值，避免重复解引用。在每次 `path_to_nameidata` 更新路径后，`nd->inode` 会同步更新
-	`link_inode`（v4.11.6 中对应 `nd->link_inode`）：当路径查找过程中遇到符号链接时，`link_inode` 记录的是符号链接文件本身的 inode（而非链接目标的 inode）。这在 `pick_link` -> `get_link` 流程中使用：内核需要调用 `inode->i_op->get_link()` 读取符号链接的目标路径，此时 `link_inode` 就是提供 `get_link` 方法的那个 inode。注意在 v4.11.6 中，该字段体现在 `saved` 结构和 `nd->stack[depth].inode` 中
...
内核通过 dentry 的标志位和类型判断函数来检测这两种情况：

-	**挂载点检测**：通过 `d_mountpoint(dentry)` 宏判断，实质是检查 `dentry->d_flags & DCACHE_MOUNTED`。当某个目录被 `mount(2)` 挂载了新文件系统后，该目录的 dentry 会被设置 `DCACHE_MOUNTED` 标志（在 `attach_mnt` 中通过 `d_set_mounted` 完成）。路径查找中，`step_into` -> `handle_mounts` 会检查此标志，若为挂载点则调用 `__follow_mount_rcu`（RCU模式）或 `follow_managed`（ref-walk模式）穿越到新文件系统的根 dentry

-	**符号链接检测**：通过 `d_is_symlink(dentry)` 判断，实质是检查 `dentry->d_flags & DCACHE_SYMLINK_TYPE`（由 `__d_set_inode_and_type` 根据 `inode->i_mode` 中的 `S_IFLNK` 设置）。当 `walk_component` 中的 `step_into` 发现当前分量是 symlink 时，会调用 `pick_link` 进入符号链接解析流程

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/include/linux/dcache.h#L428
static inline bool d_mountpoint(const struct dentry *dentry)
{
	return dentry->d_flags & DCACHE_MOUNTED;
}

//https://elixir.bootlin.com/linux/v4.11.6/source/include/linux/dcache.h#L470
...
`open` 系统调用的内核实现，可以分为三个层次：

1.	系统调用入口层（参数解析、fd 分配）
2.	VFS 层（路径查找 + 文件打开）
3.	具体文件系统层（inode 查找 + 文件操作）

核心路径查找集中在 `path_openat` 函数中，它通过 `path_init` 确定起点、`link_path_walk` 逐级解析中间分量、`do_last` 处理最终分量，三者构成了 open 的主干流程。当路径中遇到符号链接时，`trailing_symlink` 负责获取链接目标并重新进入 `link_path_walk`；当遇到挂载点时，`__follow_mount_rcu` / `follow_managed` 负责穿越到新文件系统。整体调用链如下：
...
[`getname_flags`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L124) 是路径名拷贝的核心函数，完成以下工作：

1.	通过 `__getname()`（实质是 `kmem_cache_alloc` 从 `names_cachep` slab 缓存）分配一块 `PATH_MAX`（size `4096`）字节的内核缓冲区
2.	调用 `strncpy_from_user()` 将用户空间的路径字符串拷贝到内核缓冲区，同时检查长度不超过 `PATH_MAX`
3.	初始化 `struct filename` 结构体，记录路径字符串指针、引用计数等
4.	如果启用了审计（audit），还会通过 `audit_getname()` 记录文件名用于安全审计

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L124
struct filename *getname_flags(const char __user *filename, int flags, int *empty)
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L209
...
`struct filename` 的设计意义在于：路径字符串在整个 `open` 过程中可能被多处引用（如审计子系统），通过引用计数（`refcnt`）管理生命周期，避免重复拷贝

####	nameidata 字段在路径查找各阶段的变化

以 `open("/mnt/usb/data.txt", O_RDONLY)` 为例（其中 `/mnt/usb` 是 ext4 挂载点），追踪 `nameidata` 核心字段在各阶段的变化：

| 阶段 | `nd.path.mnt` | `nd.path.dentry` | `nd.inode` | `nd.last` | `nd.flags` |
|------|---------------|-------------------|------------|-----------|------------|
| `path_init` 完成 | rootfs 的 vfsmount | `/` 根 dentry | root inode | - | `LOOKUP_RCU \| LOOKUP_FOLLOW` |
| `link_path_walk` 解析 `mnt` | rootfs 的 vfsmount | `mnt` 的 dentry | mnt 目录 inode | `"mnt"` | 同上 |
| `link_path_walk` 解析 `usb` | **usb_ext4 的 vfsmount** | **usb_ext4 根 dentry** | usb 根 inode | `"usb"` | 同上 + `LOOKUP_JUMPED` |
| `link_path_walk` 返回 | usb_ext4 的 vfsmount | usb_ext4 根 dentry | usb 根 inode | `"data.txt"` | 同上 |
| `do_last: lookup_fast` 成功 | usb_ext4 的 vfsmount | `data.txt` 的 dentry | data.txt inode | `"data.txt"` | 同上 |
| `do_last: vfs_open` 完成 | usb_ext4 的 vfsmount | `data.txt` 的 dentry | data.txt inode | `"data.txt"` | 同上 |

注意第 3 行：解析 `usb` 时发现其是挂载点，`__follow_mount_rcu` 将 `nd.path.mnt` 从 rootfs 切换到 usb_ext4 的 vfsmount，`nd.path.dentry` 从 rootfs 下 `usb` 目录的 dentry 切换到 usb_ext4 文件系统的根 dentry

####	性能视角：RCU-walk 的优势

RCU-walk 模式的核心性能优势在于完全避免了 **共享数据的写操作**，具体体现在：

1.	**无 atomic 操作**：ref-walk 每处理一个分量都需要 `dget`（`lockref_get_not_dead`，包含 `atomic_inc`）和 `dput`（包含 `atomic_dec_and_test`），这些 atomic 操作在多核系统上需要 MESI 协议的 cacheline 独占，代价高昂。RCU-walk 完全不修改引用计数
2.	**无 cacheline bounce**：ref-walk 中多个 CPU 并发查找同一路径（如 `/usr/lib/`）时，对 `dentry->d_lockref` 的竞争写会导致 cacheline 在多核之间反复失效（bounce），严重降低性能。RCU-walk 的读操作不会触发 cacheline 失效
3.	**无锁竞争**：ref-walk 中 `__d_lookup` 使用 `spin_lock` 保护哈希桶，多个线程查找同一哈希桶时需要排队。RCU-walk 的 `__d_lookup_rcu` 完全无锁
4.	**代价**：RCU-walk 需要在每个分量查找后通过 `read_seqcount_retry` 验证数据一致性，验证失败时回退到 ref-walk 重新开始，但在无竞争的常见场景下（绝大多数路径查找），验证总是成功的，因此 RCU-walk 的摊销成本极低

todo

####	安全视角：open 中的关键权限检查

`open` 系统调用在路径查找的各个阶段都穿插了安全检查，主要包括：

| 检查函数 | 调用位置 | 检查内容 |
|---------|---------|---------|
| `may_lookup` | `link_path_walk` 每个中间分量 | 当前进程对目录的执行权限（`MAY_EXEC`），即是否允许遍历该目录 |
| `may_follow_link` | `trailing_symlink` / `pick_link` | `protected_symlinks` sysctl 策略检查，防止 sticky 目录下的 symlink 攻击 |
| `may_open` | `do_last` 最终打开前 | 综合检查：只读文件系统写权限、`MAY_READ`/`MAY_WRITE`/`MAY_APPEND`、`inode_permission`（DAC + LSM） |
| `may_create` | `do_last` O_CREAT 路径 | 检查父目录的写权限和执行权限，确保允许在该目录下创建文件 |
| `security_inode_follow_link` | `get_link` | LSM（如 SELinux/AppArmor）对符号链接跟随的策略检查 |
| `inode_permission` | `may_lookup` / `may_open` 内部 | POSIX DAC（用户/组/其他权限位）+ LSM 安全模块回调 |
...
-	如果在`link_path_walk`每个分量解析过程中，中间分量是一个挂载点，`step_into` -> `handle_mounts` 会穿越到新文件系统的根 dentry（通过 `__follow_mount_rcu` 或 `follow_managed`）；如果中间分量是一个符号链接，`step_into` -> `pick_link` 会获取链接目标路径，将未解析的剩余部分压入 `nd->stack`，然后以链接目标作为新的路径继续 `link_path_walk` 的循环
...
综合来看，单个分量的查找决策流程为：`walk_component` 先判断是否为 `.`/`..`（调用 `handle_dots`），若不是则进入正常查找——首先尝试 `lookup_fast`（RCU 模式下用 `__d_lookup_rcu` 无锁查找 dcache，ref-walk 模式下用 `__d_lookup` 加锁查找），若 dcache 命中则直接得到 dentry；若未命中（返回 `0`）则回退到 `lookup_slow`，获取父目录 inode 的 `i_mutex` 锁，调用 `d_alloc_parallel` 分配 dentry，再调用文件系统的 `inode->i_op->lookup`（如 `ext4_lookup`）从磁盘读取目录项。无论哪条路径，最终都通过 `step_into` 处理挂载点穿越和符号链接检测，再由 `path_to_nameidata` 将结果更新到 `nd`
...
`path_init` 的核心逻辑分为以下几个关键分支：

1.	**RCU 模式初始化**：如果 `flags` 包含 `LOOKUP_RCU`，调用 `rcu_read_lock()` 进入 RCU 读临界区，设置 `nd->seq` 和 `nd->m_seq`（mount 序列号），后续路径查找在 RCU 保护下无锁进行
2.	**绝对路径**（以 `/` 开头）：将 `nd->path` 设置为进程的根目录 `current->fs->root`（`<root_vfsmount, root_dentry>`），同时保存到 `nd->root` 供后续 `..` 回退时使用
3.	**相对路径 + `AT_FDCWD`**：将 `nd->path` 设置为进程的当前工作目录 `current->fs->pwd`
4.	**相对路径 + 指定 `dfd`**：通过	 `fdget_raw(dfd)` 获取文件描述符对应的 `struct file`，从 `file->f_path` 中取得起始路径
5.	`nd->inode` 初始化为起始 dentry 的 `d_inode`，`nd->flags` 设置查找模式标志
...
`link_path_walk` 的循环逻辑如下：

1.	**跳过前导 `/`**：路径字符串可能以多个 `/` 开头（如 `///usr/bin`），先跳过所有前导`/`符号
2.	**逐分量循环**：对路径中的每个分量（以 `/` 分隔的名称）执行以下操作：
	-	调用 `hash_name()` 计算当前分量的哈希值和长度，结果存入 `nd->last`（`struct qstr`）和 `nd->last_type`
	-	检查当前分量类型：如果是 `.` 则设置 `LAST_DOT`，如果是 `..` 则设置 `LAST_DOTDOT`，否则为 `LAST_NORM`
	-	调用 `may_lookup(nd)` 检查当前目录的执行权限（`MAY_EXEC`），确保进程有权遍历此目录
	-	调用 `walk_component(nd, WALK_FOLLOW)` 查找当前分量对应的 dentry/inode，并处理挂载点穿越和符号链接跟随
	-	如果 `walk_component` 返回表示遇到了符号链接（`nd->last_type == LAST_BIND`），需要处理嵌套符号链接
3.	**最后分量处理**：当检测到分量后面没有更多 `/`（即到达最后一个分量时），将该分量信息保存在 `nd->last` 中，然后返回 `0`，由调用者（`do_last`）处理最后分量
4.	**返回值**：成功返回 `0`（最后分量信息保存在 `nd->last`），失败返回负的错误码
...
以 `open("/dev/binder", O_RDONLY)` 为例，`path_openat` 中 `while` 循环的两次迭代过程如下：

-	**第一次循环**：`s = "/dev/binder"`，`link_path_walk` 解析中间分量 `dev`（通过 `walk_component` 查找 dcache 得到 `dev` 的 dentry），遇到最后分量 `binder` 时退出循环，将 `binder` 保存到 `nd->last`。随后 `do_last` 处理最后分量 `binder`，通过 `lookup_fast` / `lookup_slow` 找到 `binder` 的 dentry，在 `step_into` 中发现 `binder` 是一个符号链接（`d_is_symlink` 为真），`do_last` 返回 `1` 表示需要跟随链接
-	**`trailing_symlink` 处理**：`path_openat` 调用 `trailing_symlink(nd)`，内部调用 `get_link(nd)` 读取符号链接目标（如 `binderfs/binder`），返回新路径字符串 `s = "binderfs/binder"`
-	**第二次循环**：`s = "binderfs/binder"`，`link_path_walk` 解析中间分量 `binderfs`，发现其 dentry 设置了 `DCACHE_MOUNTED` 标志（因为 binderfs 挂载在此），`step_into` -> `handle_mounts` -> `__follow_mount_rcu` 穿越到 binderfs 的根 dentry。然后最后分量 `binder` 交给 `do_last` 处理，这次 `binder` 是 binderfs 下的设备文件（非 symlink），`do_last` 成功完成 `vfs_open`
...
-	`handle_dots`[函数](https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1679)：处理当前中间节点（分量）为`.`或者`..`的场景。对于 `.` 直接返回 `0`（不做任何操作，当前路径不变）；对于 `..`，RCU 模式下调用 `follow_dotdot_rcu`、ref-walk 模式下调用 `follow_dotdot`，将 `nd->path` 回退到父目录。回退时需要特殊处理 mount 边界，即如果当前 dentry 是某个文件系统的根 dentry（`nd->path.dentry == nd->path.mnt->mnt_root`），则先通过 `mnt->mnt_parent` 回到父 mount，再从 `mnt->mnt_mountpoint` 找到挂载点的父 dentry，然后继续向上回退
...
`walk_component` 中 `lookup_fast` 的返回值决定了后续的查找路径：

-	`err > 0`（即 `1`）：dcache 命中，`path.dentry` 已指向目标分量的 dentry，`inode` 和 `seq` 已填充。直接跳过 `lookup_slow`，进入 `step_into` 处理挂载点和符号链接
-	`err == 0`：dcache 未命中（RCU 模式下 `__d_lookup_rcu` 返回 NULL，或 ref-walk 模式下 `__d_lookup` 返回 NULL）。此时需要回退到 `lookup_slow`：先通过 `unlazy_walk`（若仍在 RCU 模式）切换到 ref-walk，然后获取父目录 `inode->i_mutex`，调用 `__lookup_hash` -> `d_alloc_parallel` + `inode->i_op->lookup` 从文件系统读取目录项
-	`err < 0`：发生错误，直接向上层返回。典型情况是 `-ECHILD`：RCU 模式下 `seqcount` 验证失败（有并发修改），此错误会逐层传递到 `path_openat`，触发从 RCU-walk 全局回退到 ref-walk 模式重新开始（）
...
static int walk_component(struct nameidata *nd, int flags)
{
	...
	if (unlikely(err <= 0)) {
		...
		// follow_managed：ref-walk 模式下处理挂载点穿越和自动挂载（autofs）
		// 检查 dentry 是否设置了 DCACHE_MANAGED_DENTRY 标志（包含 DCACHE_MOUNTED / DCACHE_MANAGE_TRANSIT 等）
		// 如果是挂载点，调用 follow_mount 循环穿越直到非挂载点；如果是自动挂载点，触发 d_manage 回调
...
OK:
		...
		if (err < 0)
			return err;	//walk_component，发生错误，透传上层path_lookupat
		......
	}
}

//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L2269
			   ...
			   struct path *path, struct path *root)
{
	...
	if (unlikely(retval == -ECHILD))
		retval = path_lookupat(&nd, flags, path);	//不带LOOKUP_RCU的查找，即ref-walk模式
	...
	if (nd->flags & LOOKUP_RCU) {
		...
		*seqp = seq;
		...
		if (likely(status > 0)) {
			...
			// __follow_mount_rcu：RCU 模式下无锁穿越挂载点
			// 通过 __lookup_mnt(path->mnt, path->dentry) 在 mount_hashtable 中查找
			// 如果找到子 mount，将 path->mnt 切换到子 mount 的 vfsmount，path->dentry 切换到子 mount 的 mnt_root
			// 循环处理直到没有更多嵌套挂载（支持 mount --bind 的多层嵌套）
...
-	`d_revalidate(dentry, flags)`：验证从 dcache 中找到的 dentry 是否仍然有效。对于本地文件系统（如 ext4），`dentry->d_op->d_revalidate` 通常为 `NULL`，`d_revalidate` 直接返回 `1`（有效）。对于网络文件系统（如 NFS），服务端文件可能被其他客户端修改，`d_revalidate` 会向服务器发起验证请求。若返回 `0` 表示 dentry 已失效，需要调用 `d_invalidate` 将其从 dcache 中移除，然后走 `lookup_slow` 重新查找
-	`d_is_negative(dentry)`：检查 dentry 是否为"负 dentry"（negative dentry），即 `dentry->d_inode == NULL`。负 dentry 表示该路径分量曾被查找但不存在（如之前 `open` 一个不存在的文件），VFS 会缓存这个"不存在"的信息以加速后续查找。在 `lookup_fast` 中，如果命中了负 dentry，需要返回 `ENOENT` 或回退到 `lookup_slow`（可能文件已被创建）
-	`follow_managed(path, nd)`：ref-walk 模式下处理已挂载的挂载点和自动挂载（autofs）。它检查 `dentry->d_flags` 中的 `DCACHE_MANAGED_DENTRY` 标志（包含 `DCACHE_MOUNTED`、`DCACHE_NEED_AUTOMOUNT`、`DCACHE_MANAGE_TRANSIT`），如果是挂载点则调用 `follow_mount` 循环穿越到子文件系统的根 dentry，如果是自动挂载点则触发 `d_manage` / `d_automount` 回调完成按需挂载
...
2.	**并发 `mount` / `umount`**：另一个 CPU 正在对当前 dentry 执行挂载或卸载操作（`attach_mnt` / `umount_tree`），修改了 dentry 的 `DCACHE_MOUNTED` 标志或关联的 mount 结构
3.	**并发 `unlink` / `rmdir`**：另一个 CPU 删除了当前查找到的 dentry 对应的文件或目录（`dentry_unlink_inode` 会修改 `d_inode` 为 NULL）
4.	**dentry 被回收**：内存压力下 slab 回收器（`shrink_dcache_sb`）正在释放 dentry，可能导致 `d_lockref` 状态变化
5.	**并发 `chmod` / `chown`**：修改了 inode 的属性，虽然不直接改变 dentry 的 seqcount，但在某些文件系统中可能间接触发 dentry 状态更新

这些并发修改的存在正是 RCU-walk 需要 `seqcount` 验证的根本原因，RCU-walk 以无锁方式读取数据，必须在读取完成后验证数据的一致性
...
更具体地说，两者的区别体现在作用域和生命周期上，`lookup_fast` 的 RCU 保护覆盖整个路径查找过程（从 `path_init` 的 `rcu_read_lock()` 到查找结束或模式切换），期间不能有任何阻塞操作；而 `d_alloc_parallel` 的 RCU 仅用于内部哈希表遍历的一小段代码，遍历结束后即退出 RCU 临界区，后续的 dentry 分配（`kmem_cache_alloc`）和 inode 查找（`inode->i_op->lookup`）都在 RCU 之外执行，可以安全地阻塞。这也解释了为什么 `d_alloc_parallel` 可以在 ref-walk 的 `lookup_slow` 中被调用（其对 RCU 使用是局部且短暂的）

`lookup_slow`的实现逻辑梳理如下（有些绕）：

[`lookup_slow`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1616) 的核心流程为：

1.	先（必须）获取父目录 inode 的互斥锁：`inode_lock(dir->d_inode)`，保证同一目录下的并发查找串行化
2.	调用 `d_alloc_parallel(dir, name)` 在 dcache 中查找或分配 dentry：
	-	如果 dcache 中已存在该 dentry（被并发线程创建），直接返回已有 dentry
	-	如果不存在，分配新 dentry 并标记为 `DCACHE_PAR_LOOKUP`（在 lookup 哈希链表中）
3.	检查新 dentry 是否需要调用文件系统查找：`d_in_lookup(dentry)` ，为`true`表示需要
4.	调用 `dir->d_inode->i_op->lookup(dir->d_inode, dentry, flags)` 进入具体文件系统的查找（如 `ext4_lookup`），从磁盘读取目录项并绑定 inode
5.	调用 `d_lookup_done(dentry)` 将 dentry 从 lookup 哈希链表中移除，唤醒等待的并发线程
6.	释放父目录 inode 锁：`inode_unlock(dir->d_inode)`

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1616
static struct dentry *lookup_slow(const struct qstr *name,
				  struct dentry *dir, unsigned int flags)
{
	...
	inode_lock_shared(inode);	// 获取父目录 inode 的共享锁
	dentry = d_alloc_parallel(dir, name, &wq);	// 查找或分配 dentry
	if (IS_ERR(dentry))
		goto out;
	if (unlikely(!d_in_lookup(dentry))) {
		// 已存在于 dcache 中（并发线程已完成查找）
		if (!(flags & LOOKUP_NO_REVAL)) {
			int error = d_revalidate(dentry, flags);
			// ... 验证逻辑 ...
		}
	} else {
		// 新分配的 dentry，需要调用文件系统 lookup
		old = inode->i_op->lookup(inode, dentry, flags);
		d_lookup_done(dentry);	// 完成查找，唤醒等待者
		if (unlikely(old)) {
			dput(dentry);
			dentry = old;
		}
...
todo：d_alloc_parallel
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/dcache.c#L2405
...
既然如此，那么有个问题，在RCU模式下的`link_path_walk`函数，当进入到`!d_can_lookup(nd->path.dentry)`的分支且`unlazy_walk`成功（`unlazy_walk`返回0，最终返回`-ENOTDIR`），为何不再回退到ref-walk模式了呢？分析如下：

原因在于 `-ENOTDIR` 是一个**语义层面的永久性错误**，而非并发导致的暂态错误。`d_can_lookup(dentry)` 检查的是 `dentry->d_flags & DCACHE_DIRECTORY_TYPE`，即该 dentry 对应的 inode 是否为目录类型。这是 inode 的固有属性（由 `i_mode` 中的 `S_IFDIR` 决定），不会因为查找模式的不同而改变，即一个文件不会因为从 RCU-walk 切换到 ref-walk 就变成目录。因此，这里 `unlazy_walk` 的目的仅是安全退出 RCU 模式（获取必要的引用计数，避免资源泄漏），而非为了重试查找
...
| 维度 | RCU-walk 模式 | ref-walk 模式 |
|------|--------------|--------------|
| 触发条件 | `do_filp_open` 首次调用 `path_openat` 时默认使用（`flags = LOOKUP_RCU`） | RCU-walk 返回 `-ECHILD` 后，`do_filp_open` 清除 `LOOKUP_RCU` 重新调用 `path_openat` |
| 锁机制 | `rcu_read_lock()` + seqcount 验证，无自旋锁、无互斥锁 | `dentry->d_lockref` 引用计数 + `inode->i_rwsem` 互斥锁 |
| dcache 查找 | `lookup_fast` -> `__d_lookup_rcu`（无锁遍历哈希桶，seqcount 验证） | `lookup_fast` -> `__d_lookup`（`spin_lock` 保护，`dget` 增加引用计数） |
| 缓存未命中 | 返回 `0`，触发 `unlazy_walk` 切换到 ref-walk 后走 `lookup_slow` | 直接调用 `lookup_slow`（`inode_lock` -> `d_alloc_parallel` -> `inode->i_op->lookup`） |
| 挂载点穿越 | `__follow_mount_rcu`（无锁 `__lookup_mnt`） | `follow_managed` -> `follow_mount`（`dget`/`dput` + `__lookup_mnt`） |
| 失败处理 | 返回 `-ECHILD` 回退到 ref-walk | 返回具体错误码（`-ENOENT`、`-EACCES` 等） |
| 性能特点 | 无 atomic 操作、无 cacheline bounce，多核扩展性好 | 引用计数更新导致 cacheline 刷新，高并发时有锁竞争 |

```mermaid
flowchart TD
    A["do_filp_open"] --> B["path_openat(LOOKUP_RCU)"]
    B --> C{"成功?"}
    C -->|"是"| D["返回 file"]
    C -->|"-ECHILD"| E["path_openat(ref-walk)"]
    E --> F{"成功?"}
    F -->|"是"| D
    F -->|"-ESTALE"| G["path_openat(LOOKUP_REVAL)"]
    G --> D
```
...
-	**普通文件**：如 `open("/home/user/data.txt", O_RDONLY)`，最后分量 `data.txt` 是一个常规文件
-	**目录**：如 `open("/home/user/docs/", O_RDONLY | O_DIRECTORY)`，最后分量 `docs` 是一个目录
-	**符号链接**：如 `open("/etc/localtime", O_RDONLY)`，`localtime` 可能是指向 `/usr/share/zoneinfo/...` 的 symlink
-	**不存在的路径**：如 `open("/tmp/newfile", O_CREAT | O_WRONLY, 0644)`，最后分量 `newfile` 尚不存在，需要创建
-	**特殊文件**：设备节点（如 `/dev/null`、`/dev/sda`）、FIFO（命名管道）、UNIX 域 socket 等
-	**挂载点**：最后分量本身就是另一个文件系统的挂载点（如 `open("/mnt/usb/", O_RDONLY)`）
...
####	case3：跨越 mount 点打开文件

场景：`open("/mnt/usb/file.txt", O_RDONLY)`，其中 `/mnt/usb` 是一个 ext4 文件系统的挂载点（U 盘挂载），`file.txt` 是该 ext4 文件系统中的文件

在 `do_last` 中处理最后分量 `file.txt` 时，关键路径如下：

1.	`link_path_walk` 阶段已完成中间分量 `mnt` 和 `usb` 的解析。在解析 `usb` 时，`step_into` -> `handle_mounts` 检测到 `usb` 的 dentry 设置了 `DCACHE_MOUNTED` 标志，通过 `__follow_mount_rcu`（RCU 模式）或 `follow_managed`（ref-walk 模式）穿越到 U 盘 ext4 文件系统的根 dentry。此时 `nd->path` 已指向 `<usb_vfsmount, usb_root_dentry>`
2.	`do_last` 开始处理最后分量 `file.txt`。由于没有 `O_CREAT` 标志，走非创建路径
3.	调用 `lookup_fast` 在 dcache 中查找 `file.txt`：以 U 盘文件系统的根 dentry 为父目录，在哈希表中查找名为 `file.txt` 的子 dentry
4.	如果 dcache 命中，`lookup_fast` 返回 `1`，`path.dentry` 指向 `file.txt` 的 dentry
5.	进入 `step_into`，`handle_mounts` 检查 `file.txt` 的 dentry 是否也是挂载点——通常不是（它是普通文件），直接跳过
6.	`step_into` -> `path_to_nameidata` 更新 `nd->path` 指向 `file.txt`
7.	继续 `do_last` 的后续流程：权限检查（`may_open`）-> `vfs_open` -> `ext4_file_open`

核心源码路径（`do_last` 中非创建分支）：

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L3259
		   ...
		   const struct open_flags *op, int *opened)
{
	...
	if (!(open_flag & O_CREAT)) {
		...
		if (nd->last.name[nd->last.len])
			nd->flags |= LOOKUP_FOLLOW | LOOKUP_DIRECTORY;
		// 在 dcache 中查找 file.txt
		error = lookup_fast(nd, &path, &inode, &seq);
		if (likely(error > 0))
			goto finish_lookup;	// dcache 命中，跳到完成查找
		// ... lookup_slow 回退路径 ...
	}
	// ...
finish_lookup:
	// step_into 内部调用 handle_mounts 处理挂载点穿越
	error = step_into(nd, &path, flags, inode, seq);
	// ... 权限检查 + vfs_open ...
}
```
...
```mermaid
flowchart TD
    A["do_last: 处理 file.txt"] --> B{"O_CREAT?"}
    B -->|"否"| C["lookup_fast 查找 dcache"]
    C -->|"命中"| D["finish_lookup"]
    C -->|"未命中"| E["lookup_slow"]
    E --> D
    D --> F["step_into"]
    F --> G["handle_mounts: 检查挂载点"]
    G -->|"非挂载点"| H["path_to_nameidata"]
    G -->|"是挂载点"| I["__follow_mount_rcu 穿越"]
    I --> H
    H --> J["may_open: 权限检查"]
    J --> K["vfs_open -> ext4_file_open"]
```

在此场景中，mount 穿越主要发生在 `link_path_walk` 阶段（解析中间分量 `usb` 时），而 `do_last` 阶段对 `file.txt` 的挂载点检查通常直接通过（因为普通文件不是挂载点）

####	case4：最后分量为 symlink 的场景

如场景`open("/etc/localtime", O_RDONLY)`，其中 `/etc/localtime` 是指向 `/usr/share/zoneinfo/Asia/Shanghai` 的符号链接

在 `do_last` 中处理最后分量 `localtime` 时，会触发符号链接跟随机制：

1.	`link_path_walk` 解析中间分量 `etc`，`nd->path` 指向 `<rootfs_vfsmount, etc_dentry>`
2.	`do_last` 开始处理最后分量 `localtime`，通过 `lookup_fast` 或 `lookup_slow` 找到 `localtime` 的 dentry
3.	进入 `step_into`，调用 `d_is_symlink(path->dentry)` 检测到 `localtime` 是符号链接
4.	由于 `nd->flags` 包含 `LOOKUP_FOLLOW`（默认行为），`step_into` 调用 `pick_link` 进入符号链接处理
5.	`pick_link` 内部调用 `may_follow_link` 进行安全检查（检查 `protected_symlinks` sysctl 策略），然后调用 `get_link` -> `inode->i_op->get_link()` 读取符号链接目标路径 `/usr/share/zoneinfo/Asia/Shanghai`
6.	`do_last` 返回值 `> 0`（表示需要跟随链接），回到 `path_openat` 的 `while` 循环
7.	`path_openat` 调用 `trailing_symlink(nd)` 获取符号链接目标字符串 `s`
8.	由于目标以 `/` 开头（绝对路径），`nd->path` 重置为根目录，然后重新进入 `link_path_walk` 解析 `/usr/share/zoneinfo/Asia/Shanghai`

核心源码路径：

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L3457
static struct file *path_openat(struct nameidata *nd,
			const struct open_flags *op, unsigned flags)
{
	// ...
	while (!(error = link_path_walk(s, nd)) &&
		(error = do_last(nd, file, op, &opened)) > 0) {
		// do_last 返回 > 0 表示遇到了符号链接需要跟随
		nd->flags &= ~(LOOKUP_OPEN|LOOKUP_CREATE|LOOKUP_EXCL);
		s = trailing_symlink(nd);	// 获取链接目标路径
		if (IS_ERR(s)) {
			error = PTR_ERR(s);
			break;
		}
		// s 现在是 "/usr/share/zoneinfo/Asia/Shanghai"
		// 回到 while 循环顶部，重新执行 link_path_walk
	}
	// ...
}
```

`trailing_symlink`的实现：

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L3421
static const char *trailing_symlink(struct nameidata *nd)
{
	const char *s;
	int error = may_follow_link(nd);	// 安全检查
	if (unlikely(error))
		return ERR_PTR(error);
	nd->flags |= LOOKUP_PARENT;
	nd->stack[0].name = nd->last.name;
	// 调用 get_link 读取符号链接目标
	s = get_link(nd);
	return s;
}
```

**嵌套限制**：内核通过 `MAXSYMLINKS`（v4.11.6 中定义为 `40`）限制符号链接的最大嵌套深度。每跟随一个符号链接，`nd->total_link_count` 递增，超过 `MAXSYMLINKS` 时返回 `-ELOOP`。同时 `nd->depth` 记录当前递归深度（中间分量的嵌套），`nd->stack` 数组保存每层嵌套的未解析剩余路径

```mermaid
flowchart TD
    A["do_last: 处理 localtime"] --> B["lookup_fast/slow 找到 dentry"]
    B --> C["step_into"]
    C --> D{"d_is_symlink?"}
    D -->|"否"| E["path_to_nameidata -> vfs_open"]
    D -->|"是"| F["pick_link"]
    F --> G["may_follow_link: 安全检查"]
    G --> H["get_link: 读取链接目标"]
    H --> I["do_last 返回 > 0"]
    I --> J["path_openat: trailing_symlink"]
    J --> K["重新 link_path_walk 解析目标路径"]
    K --> L["第二轮 do_last 处理 Shanghai"]
    L --> E
```
...
[`follow_managed`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1199) 是 ref-walk 模式下处理挂载点穿越和自动挂载的核心函数。当路径查找在 ref-walk 模式下找到一个 dentry 后，需要检查该 dentry 是否是挂载点或自动挂载点，并进行相应的穿越操作：

todo

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1199
static int follow_managed(struct path *path, struct nameidata *nd)
{
	...
	while (managed = ACCESS_ONCE(path->dentry->d_flags),
	       managed &= DCACHE_MANAGED_DENTRY,	// 检查是否为受管理的 dentry（挂载点/自动挂载等）
	       unlikely(managed != 0)) {
		if (managed & DCACHE_MANAGE_TRANSIT) {
			// 调用文件系统的 d_manage 回调（autofs 用于阻塞等待挂载完成）
		...
		if (managed & DCACHE_MOUNTED) {
			// 当前 dentry 是挂载点，调用 __follow_mount 穿越到子文件系统
			struct vfsmount *mounted = lookup_mnt(path);
			if (mounted) {
				dput(path->dentry);		// 释放旧 dentry
				if (need_mntput)
					mntput(path->mnt);
				path->mnt = mounted;		// 切换到子 mount
				path->dentry = dget(mounted->mnt_root);	// 获取子文件系统根 dentry
				need_mntput = true;
				continue;	// 继续检查（支持多层嵌套挂载）
			}
		}
		if (managed & DCACHE_NEED_AUTOMOUNT) {
			// 自动挂载点：触发 d_automount 回调进行按需挂载
...
该函数通过 `while` 循环支持多层嵌套挂载的穿越（如 A 挂载在 B 上，B 又挂载在 C 上），直到找到一个非挂载点的 dentry 为止
...
`handle_dots` 处理路径分量中的 `.` 和 `..` 两种特殊分量。对于 `.`（`LAST_DOT`），直接返回 `0`，不做任何操作，当前路径保持不变。对于 `..`（`LAST_DOTDOT`），需要将路径回退到父目录，这涉及两个关键问题：

1.	**跨越 mount 边界**：如果当前 dentry 恰好是某个文件系统的根 dentry（`nd->path.dentry == nd->path.mnt->mnt_root`），不能简单地取 `dentry->d_parent`，而是要先回到父 mount（通过 `mnt->mnt_parent`），然后从父 mount 的挂载点 dentry（`mnt->mnt_mountpoint`）继续向上走
2.	**根目录边界**：到达进程的根目录（`nd->root`）后不能再向上，`..` 操作被忽略（即 `/..` 等价于 `/`）
...
static inline int handle_dots(struct nameidata *nd, int type)
{
	if (type == LAST_DOTDOT) {
		if (!nd->root.mnt)
			set_root(nd);	// 延迟初始化根目录（相对路径场景）
		if (nd->flags & LOOKUP_RCU) {
			return follow_dotdot_rcu(nd);	// RCU 模式：无锁回退
		} else
			return follow_dotdot(nd);	// ref-walk 模式：带引用计数回退
	}
	return 0;	// LAST_DOT：什么都不做
}
```

`follow_dotdot_rcu` 的核心逻辑（[源码](https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1354)）：
...
static int follow_dotdot_rcu(struct nameidata *nd)
{
	// 循环处理：可能需要穿越多层 mount 边界
	while (1) {
		if (path_equal(&nd->path, &nd->root))
			break;	// 已到达根目录，停止回退
		if (nd->path.dentry != nd->path.mnt->mnt_root) {
			// 未到 mount 根：直接取父 dentry
			struct dentry *old = nd->path.dentry;
			struct dentry *parent = old->d_parent;
			// ... seqcount 验证 ...
			nd->path.dentry = parent;
			...
			nd->path.mnt = &mparent->mnt;
			nd->path.dentry = mountpoint;
			// 不 break，继续循环（mountpoint 可能还是某个 mount 的根）
		}
	}
	// 更新 nd->inode
	nd->inode = nd->path.dentry->d_inode;
...
`unlazy_walk` 的成功与失败条件总结：

todo

-	**成功条件**（返回 `0`）：当前 `nd->path.dentry` 的 seqcount 验证通过（`read_seqcount_retry` 返回 `0`）、`lockref_get_not_dead` 成功获取引用计数（dentry 未被释放）、`__legitimize_mnt` 成功获取 mount 引用、如果有嵌套符号链接则 `legitimize_links` 验证所有 `nd->stack` 中的链接路径有效。成功后清除 `nd->flags` 中的 `LOOKUP_RCU`，调用 `rcu_read_unlock()` 退出 RCU 临界区
-	**失败条件**（返回 `-ECHILD`）：上述任何一步验证失败，seqcount 在读取期间被修改（并发 rename/mount/unlink）、dentry 引用计数已为死亡状态（并发释放）、mount 序列号验证失败（并发 mount/umount）、嵌套符号链接的路径验证失败。失败后同样退出 RCU 临界区，但调用者需要将错误向上传播，最终导致 `path_openat` 返回 `-ECHILD`，由 `do_filp_open` 发起全局回退
...
从 `struct mount` 的成员来看，**反向定位**是可以的：每个 `mount` 结构都记录了 `mnt_parent`（父 mount）和 `mnt_mountpoint`（在父文件系统中的挂载点 dentry），因此从一个子 mount 出发可以直接找到其父 mount 和挂载点位置。`follow_dotdot` / `follow_dotdot_rcu` 处理 `..` 时就是利用这种反向关系穿越 mount 边界的。

但是**正向查找**（即给定一个 `<parent_vfsmount, dentry>` 判断是否有子文件系统挂载在此）仍然需要 `mount_hashtable`。原因是：一个 dentry 上可能挂载了多个文件系统（堆叠挂载），也可能没有挂载任何文件系统。仅通过 dentry 的 `DCACHE_MOUNTED` 标志可以判断"是否有挂载"，但具体找到是哪个 `mount` 实例，需要以 `<parent_vfsmount, dentry>` 为 key 在 `mount_hashtable` 中查找。这就是 `__lookup_mnt` 的作用，即正向查找的唯一路径
...
struct mount *__lookup_mnt(struct vfsmount *mnt, struct dentry *dentry)
{
	struct hlist_head *head = m_hash(mnt, dentry);	// 以 <mnt, dentry> 计算哈希桶
	struct mount *p;
	hlist_for_each_entry_rcu(p, head, mnt_hash)
		if (&p->mnt_parent->mnt == mnt && p->mnt_mountpoint == dentry)
			return p;	// 找到子 mount
...
####	`__lookup_hash`的实现及应用场景

todo

[`__lookup_hash`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1468) 是一个在已持有父目录 inode 锁的情况下进行 dentry 查找或创建的内部函数。它与 `lookup_slow` 的区别在于**`lookup_slow` 需要自行获取 inode 锁，而 `__lookup_hash` 假设调用者已持有锁**

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1468
static struct dentry *__lookup_hash(const struct qstr *name,
		struct dentry *base, unsigned int flags)
{
	struct dentry *dentry = lookup_dcache(name, base, flags);  // 先查 dcache
	if (dentry)
		return dentry;	// dcache 命中，直接返回
	// dcache 未命中，分配新 dentry 并调用文件系统 lookup
	dentry = d_alloc(base, name);
	if (unlikely(!dentry))
		return ERR_PTR(-ENOMEM);
	return lookup_real(base->d_inode, dentry, flags);  // 调用 inode->i_op->lookup
}
```

`__lookup_hash` 主要在以下场景中被调用：`do_last` 中处理 `O_CREAT` 标志时（`lookup_open` 内部），以及 `mkdir` / `mknod` / `link` 等需要在已锁定目录中查找目标分量的系统调用中。相比 `lookup_slow` 使用的 `d_alloc_parallel`（支持并发查找去重），`__lookup_hash` 使用更简单的 `d_alloc`，因为调用者已经持有了目录的互斥锁，不存在并发查找同一分量的情况
...
##	0x08	open 中涉及到的 mount 操作

在 `open` 系统调用的路径查找过程中，每当解析到一个分量的 dentry，都需要检查该 dentry 是否为挂载点。如果是，则需要"穿越"（traverse）到挂载在其上的子文件系统的根 dentry。这一过程涉及以下核心函数：

####	__lookup_mnt：挂载哈希表查找

[`__lookup_mnt`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/namespace.c#L624) 是挂载点穿越的基础函数，在 `mount_hashtable` 中查找以 `<parent_vfsmount, dentry>` 为 key 的子 mount：

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namespace.c#L624
struct mount *__lookup_mnt(struct vfsmount *mnt, struct dentry *dentry)
{
	// 以 <mnt, dentry> 计算哈希值，定位到对应的哈希桶
	struct hlist_head *head = m_hash(mnt, dentry);
	struct mount *p;

	// 遍历哈希桶中的 mount 链表（RCU 保护下无锁遍历）
	hlist_for_each_entry_rcu(p, head, mnt_hash)
		// 匹配条件：父 mount 的 vfsmount 相同 且 挂载点 dentry 相同
		if (&p->mnt_parent->mnt == mnt && p->mnt_mountpoint == dentry)
			return p;
	return NULL;
}
...
该函数在 RCU 保护下执行，无需获取任何锁，时间复杂度为 `O(1)`（哈希冲突时退化为 `O(n)`）。注意如果同一个 dentry 上有多层堆叠挂载，`__lookup_mnt` 返回最近一次挂载的 mount（即 `hlist` 头部的那个）

####	real_mount：从 vfsmount 获取 mount

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/mount.h#L54
...
`struct vfsmount` 是嵌入在 `struct mount` 内部的成员（`mount.mnt`），`real_mount` 通过 `container_of` 宏从 `vfsmount` 指针反向获取包含它的 `struct mount` 实例。这是内核中常见的面向对象设计模式，对外暴露轻量的 `vfsmount` 接口，内部维护完整的 `mount` 结构

####	__follow_mount_rcu：RCU 模式下的挂载点穿越

[`__follow_mount_rcu`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1156) 是 RCU-walk 模式下穿越挂载点的核心函数。它在 `lookup_fast` 成功找到 dentry 后被调用，用于检查并穿越可能的挂载点：

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1156
static bool __follow_mount_rcu(struct nameidata *nd, struct path *path,
			       struct inode **inode, unsigned *seqp)
{
	for (;;) {
		struct mount *mounted;
		// 检查 DCACHE_MANAGE_TRANSIT：某些特殊文件系统（如 autofs）
		// 需要阻塞等待，RCU 模式下不能阻塞，返回 false 触发模式切换
		if (unlikely(managed_dentry(path->dentry, nd->flags)))
			return false;

		// 在 mount_hashtable 中查找子 mount
		mounted = __lookup_mnt(path->mnt, path->dentry);
		if (!mounted)
			break;	// 非挂载点，退出循环

		// 穿越到子文件系统：切换 vfsmount 和 dentry
		path->mnt = &mounted->mnt;
		path->dentry = mounted->mnt.mnt_root;	// 子文件系统的根 dentry
		nd->flags |= LOOKUP_JUMPED;

		// 更新 seqcount（用于后续验证一致性）
		*seqp = read_seqcount_begin(&path->dentry->d_seq);
		// 更新 inode 缓存
		*inode = path->dentry->d_inode;

		// 继续循环：新的根 dentry 可能也是挂载点（多层嵌套挂载）
	}
	return true;
}
```

`for(;;)` 循环的设计支持多层嵌套挂载：如果 A 挂载在 B 上，C 又挂载在 A 的根目录上，那么穿越 B 的挂载点后到达 A 的根 dentry，发现它也是挂载点，继续穿越到 C 的根 dentry

####	follow_mount：ref-walk 模式下的挂载点穿越

`follow_mount` 是 ref-walk 模式下的挂载点穿越实现。与 `__follow_mount_rcu` 不同，它需要管理引用计数：

```cpp
//follow_mount 的等效逻辑（在 follow_managed 内部实现）
while (d_mountpoint(path->dentry)) {
	struct vfsmount *mounted = lookup_mnt(path);  // __lookup_mnt + mntget
	if (!mounted)
		break;
	dput(path->dentry);		// 释放旧 dentry 引用
	mntput(path->mnt);		// 释放旧 mount 引用（需要时）
	path->mnt = mounted;		// 切换到子 mount
	path->dentry = dget(mounted->mnt_root);	// 获取子根 dentry 引用
}
```

ref-walk 模式需要 `dget`/`dput` 和 `mntget`/`mntput` 管理引用计数，保证在使用 dentry 和 mount 期间它们不会被释放。这是 ref-walk 比 RCU-walk 开销更大的原因之一

####	mount 穿越的整体流程

```mermaid
flowchart TD
    A["walk_component / do_last: 找到分量 dentry"] --> B["step_into"]
    B --> C["handle_mounts"]
    C --> D{"nd->flags & LOOKUP_RCU?"}
    D -->|"是"| E["__follow_mount_rcu"]
    D -->|"否"| F["follow_managed"]

    E --> G{"__lookup_mnt 找到子 mount?"}
    G -->|"是"| H["path->mnt = mounted.mnt\npath->dentry = mnt_root"]
    G -->|"否"| I["返回: 非挂载点"]
    H --> J{"新 dentry 也是挂载点?"}
    J -->|"是"| G
    J -->|"否"| I

    F --> K{"DCACHE_MANAGED_DENTRY?"}
    K -->|"DCACHE_MANAGE_TRANSIT"| L["d_manage 回调"]
    K -->|"DCACHE_MOUNTED"| M["lookup_mnt -> 穿越"]
    K -->|"DCACHE_NEED_AUTOMOUNT"| N["follow_automount"]
    K -->|"无标志"| I
    M --> O{"继续检查嵌套?"}
    O -->|"是"| K
    O -->|"否"| I
...
##	0x09	open 中涉及到的 symlink 操作

在 `open` 系统调用的路径查找过程中，符号链接（symlink）的处理分为两种场景：**中间分量的符号链接**和**最后分量（trailing）的符号链接**

####	中间分量遇到 symlink

当 `link_path_walk` 解析中间分量时，通过 `walk_component` -> `step_into` 发现当前分量是符号链接后，处理流程如下：

1.	`step_into` 检测到 `d_is_symlink(path->dentry)` 为真
2.	如果 `flags` 不包含 `WALK_NOFOLLOW`（`link_path_walk` 中对中间分量总是设置 `WALK_FOLLOW`），则调用 `pick_link`
3.	`pick_link` 内部：
	-	递增 `nd->total_link_count`，检查是否超过 `MAXSYMLINKS`（`40`），超过则返回 `-ELOOP`
	-	递增 `nd->depth`，将当前未解析的剩余路径保存到 `nd->stack[depth]`
	-	调用 `get_link(nd)` -> `inode->i_op->get_link()` 读取符号链接目标路径
4.	`get_link` 返回符号链接目标字符串后，`link_path_walk` 以此为新的路径继续循环解析

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L1812
static int pick_link(struct nameidata *nd, struct path *link,
		     struct inode *inode, unsigned seq)
{
	int error;
	...
	last->link = *link;
	last->seq = seq;
	// 调用 get_link 获取符号链接目标
	// ...
}
```

####	尾部分量（trailing）的 symlink

（前文已讨论）当 `do_last` 处理最后分量时发现是符号链接，处理流程与中间分量不同。它不在 `link_path_walk` 内部处理，而是返回到 `path_openat` 的 `while` 循环：

1.	`do_last` 中 `step_into` 检测到 symlink，返回值 `> 0`
2.	`path_openat` 调用 `trailing_symlink(nd)` 获取链接目标
3.	`trailing_symlink` 内部调用 `may_follow_link` 进行安全检查，然后调用 `get_link` 读取目标
4.	返回目标路径字符串 `s`，`path_openat` 的 `while` 循环以 `s` 重新进入 `link_path_walk`

####	安全机制：may_follow_link

[`may_follow_link`](https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L876) 实现了 Linux 的 **protected_symlinks** 安全策略（通过 `sysctl fs.protected_symlinks` 控制）。当启用此策略时，在 sticky 目录（如 `/tmp`）中跟随符号链接需要满足以下条件之一：

-	符号链接的所有者是当前进程的 fsuid
-	符号链接所在目录的所有者是当前进程的 fsuid
-	符号链接的所有者与所在目录的所有者相同

这一机制防止了经典的 `/tmp` 符号链接攻击：恶意用户在 `/tmp` 中创建指向 `/etc/shadow` 的符号链接，诱使特权进程跟随该链接打开敏感文件

```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/fs/namei.c#L876
static int may_follow_link(struct nameidata *nd, const struct inode *inode)
{
	// 如果 protected_symlinks 未启用，直接允许
	if (!sysctl_protected_symlinks)
		return 0;
	// 符号链接所在目录不是 sticky 目录，允许
	if (!(nd->path.dentry->d_inode->i_mode & S_ISVTX))
		return 0;
	// 链接的所有者是当前进程的 fsuid，允许
	if (uid_eq(inode->i_uid, nd->path.dentry->d_inode->i_uid))
		return 0;
	if (uid_eq(current_cred()->fsuid, inode->i_uid))
		return 0;
	// 拒绝：审计并返回 -EACCES
	if (nd->flags & LOOKUP_RCU)
		return -ECHILD;	// RCU 模式下不能审计，回退到 ref-walk
...
内核通过两个计数器限制符号链接的嵌套（关联`nameidata`对象），如下：

-	`nd->total_link_count`：整个路径解析过程中跟随的符号链接总数，不超过 `MAXSYMLINKS`（`40`）。这包括中间分量和尾部分量的所有链接
-	`nd->depth`：当前符号链接的嵌套深度（中间分量），最大为 `EMBEDDED_LEVELS`（`2`）+ 动态分配。超过 `EMBEDDED_LEVELS` 时需要 `kmalloc` 分配额外的 `nd->stack` 空间

`nd->stack` 是一个 `struct saved` 数组，每个元素保存一层符号链接解析的上下文：

```cpp
struct saved {
	struct path link;	// 符号链接本身的 path（用于后续 put_link 释放）
	struct delayed_call done;
	const char *name;	// 链接解析后剩余的未解析路径
	unsigned seq;
};
```
