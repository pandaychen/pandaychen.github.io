...
-   **TCP 完整通信过程**（三次握手 / 四次挥手 / 状态机）：详见 [Linux 内核之旅（十二）：内核视角下的 TCP 完整通信过程](https://pandaychen.github.io/2025/04/25/A-LINUX-KERNEL-TRAVEL-12/)
...
4.  接收端滑动窗口与流量控制
5.  拥塞控制（双端视角下的 CUBIC/Reno 算法源码、拥塞状态机）
...
| 字段（对端发送过来的数据包） | 视角：收到对端的报文时 | 视角：收到对端的报文时 | 补充 |
| :-----| :---- | :---- |:---- |
...
####    snd_* VS rcv_*
在 TCP 双端交互中，本端的 `snd_*`（发送侧标尺）是典型的**外生驱动型变量**，强依赖于对端通过 ACK 报文捎带回来的确认号（Acknowledgment Number）和通告窗口大小（Window Size）；而本端的 `rcv_*`（接收侧标尺，主要是 `rcv_nxt` 和 `rcv_wnd`）则是**内外双重驱动型变量**，它们既受到对端发送数据行为（外部网络因素）的冲击，更受到本端应用层读取速度与内核内存限额（内部系统因素）的直接控制

1、`rcv_nxt`（期望接收的下一个序号）受什么影响？

`rcv_nxt`本质上是内核在接收流中的推进标尺，此变量位置移动必须满足进入本端协议栈的数据包是否是连续且合法的，如前文描述，对应两个场景：

-   对端发送的数据（正常按序到达）：当对端发送的 TCP Segment 到达，且其报文头中的 `seq == tp->rcv_nxt` 时，说明数据按序到达。内核会将其挂载到 `sk_receive_queue` 中，todo
-   本端乱序队列（红黑树`tp->out_of_order_queue`）的桥接成功（异步推进）：此时 `rcv_nxt` 会卡在空缺处，一旦对端重传了缺失报文，内核在 `tcp_ofo_queue()` 中发现该包正好补上了空缺，并且能与红黑树中后续已缓存的包连成一片，`rcv_nxt` 就会一口气向后跳跃移动多个包的长度

2、`rcv_wnd`（本端通告接收窗口）受什么影响？`rcv_wnd` 是内核**向对端宣告的剩余承载能力**。它代表未读数据缓冲区的上限与当前已缓存未读数据之间的差值
...
```cpp
//https://elixir.bootlin.com/linux/v4.11.6/source/net/ipv4/tcp.c#L1450
